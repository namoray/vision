...
from core import configuration, constants as core_cst
...
def base_priority(synapse: T) -> float:
    ...
    with resource_management.threading_lock:
        total_requests_so_far = sum(requests_from_each_validator.values())
        requests_from_validator = requests_from_each_validator.get(synapse.dendrite.hotkey, 0)
        requests_from_each_validator[synapse.dendrite.hotkey] = requests_from_validator + 1

    if total_requests_so_far <= 0 or total_stake <= 0:
        return 1

    proportion_of_requests_from_this_validator = requests_from_validator / total_requests_so_far
    ...
    if proportion_of_requests_from_this_validator < validator_proprtion_of_stake:
        ...
        bt.logging.debug(f"More than allowance so priority {validator_proprtion_of_stake / proportion_of_requests_from_this_validator:.4f}")
...
class CoreMiner:
    ...
    def start_serving_axon(self) -> None:
        bt.logging.info(f"Serving axon on network: {self.config.subtensor.chain_endpoint} with netuid: {self.config.netuid}")
