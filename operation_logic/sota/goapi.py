...
async def _create_image(prompt: str, sota_key: str) -> Dict[str, Any] | None:
    async with httpx.AsyncClient() as client:
        ...
        response = await client.post("https://api.midjourneyapi.xyz/mj/v2/imagine", headers=headers, json=data)
...
async def _get_image_from_task(task_id: str, sota_key: str) -> Dict[str, Any]:
    async with httpx.AsyncClient() as client:
        ...
        response = await client.post("https://api.midjourneyapi.xyz/mj/v2/fetch", headers=headers, json=json)
    return response.json()


async def get_image(prompt: str, sota_key: str) -> Optional[str]:
    create_image_response = await _create_image(prompt, sota_key)
    if create_image_response is None:
        bt.logging.error("No image task made - bad credits?")
        return None
