...
class ClipValidator(BaseValidator):
    ...
    async def query_miner_with_images(
        ...
        query = ClipEmbeddingImages(image_b64s=image_b64s)
    ...
    async def query_miner_with_texts(
        ...
        query = ClipEmbeddingTexts(text_prompts=text_prompts)
        return await self.query_miner(metagraph.axons[uid], uid, query)
    ...
    async def run_image_embedding_query_for_uid(
        self, uid: int, image_b64s: List[str], metagraph: bt.metagraph
    ) -> Tuple[int, float]:
    ...
    async def get_scores_for_image_embeddings(
        self, image_b64s: list[str], metagraph, available_uids: Dict[int, float]
    ) -> Dict[int, float]:
    ...
    async def get_scores_for_text_embeddings(self, metagraph, available_uids: List[int]) -> Dict[int, float]:
        ...
        for i in range(0, len(available_uids_list), 50):
            batch_uids = available_uids_list[i : i + 50]
            text_tasks = [
                asyncio.create_task(self.run_text_embedding_query_for_uid(uid, metagraph)) for uid in batch_uids
            ]
